...
result.addEventListener("click",()=>{
    ...
    .then(locationData=>{
        ...
        {
            throw new Error("Can not find cityy ");
    ...
    .toLocaleString();
        temprature.textContent=`Temprature : ${temp}°C`
        time.textContent=`Date&Time : ${newtime}`
        if(RainChance<25)
        {
            rainProb.textContent=`Rain chance : ${RainChance}%☀️`
        ...
        {
            rainProb.textContent=`Rain chance : ${RainChance}%🌩️🌧️`
        }
        else 
        {
            rainProb.textContent=`Rain chance : ${RainChance}%⛈️🌩️🌧️`
        }

        

        // message.textContent="Current Tamprature"
    }).catch(error=>{
        console.error(error);
        temprature.textContent="Unable to get temprature"
